...
def run_conversation(
    ...
    *,
    ...
    for turn in range(1, max_turns + 1):
        ...
        # `if tools:` not `is not None` — an empty list must be omitted too.
        # Sending "tools": [] is not the same as sending no tools at all.
        if tools:
            request["tools"] = tools
        ...
        if response.stop_reason != "tool_use":
            ...
            return ConversationResult(messages, final_text, turn, response.usage)
